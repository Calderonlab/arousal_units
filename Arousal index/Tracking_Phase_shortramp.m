function Tracking_Phase_shortramp(model,file)
...
for i=1:length(movstr)
   ...
   if i~=4
     for j=1:size(sub,1)
         plot(pts_func(sub(j,1:2))*ones(1,2),pset(i,:),'color',colors_p(i,:),'linewidth',1.2);hold on;
     end
   else
     for j=1:size(sub,1)
         x=pts_func(sub(j,1:2));
         y=pset(i,1);
         w=pts_func(sub(j,3:4))-pts_func(sub(j,1:2))+1;
         h=1;
         rectangle('Position',[x,y,w,h],'facecolor',colors_p(i,:),'edgecolor','none');hold on;
     end
   end
end
set(gca,'XLim',x_lim);
